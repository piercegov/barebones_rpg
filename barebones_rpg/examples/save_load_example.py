...
def quest_complete_callback(quest):
    ...
    print(f"Quest '{quest.name}' completed! Gained {quest.exp_reward} EXP and {quest.gold_reward} gold!")
...
def main():
    ...
    hero.inventory.add_item(create_consumable("Health Potion", on_use=heal_potion, value=20))
    hero.inventory.add_item(create_consumable("Mana Potion", on_use=mana_potion, value=15))
    
    # Register hero
    game.register_entity(hero)
    print(f"   Created hero: {hero.name} (Level {hero.stats.level})")
    print(f"   Inventory: {len(hero.inventory.items)} items, {hero.inventory.gold} gold")
    ...
    print(f"   Created quest: {quest.name} ({quest.get_progress_percentage()*100:.0f}% complete)\n")
    ...
    if success:
        ...
        if loaded_quest:
            print(f"   Quest: {loaded_quest.name} ({loaded_quest.get_progress_percentage()*100:.0f}% complete)")
    ...
    if loaded_hero and loaded_hero.inventory:
        ...
        if health_potion and health_potion.on_use:
            print(f"   Found Health Potion with callback: {health_potion.on_use.__name__}")
